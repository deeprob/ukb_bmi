...
def main(phenos_of_interest_file, exome_file, pheno_info_root, pheno_storage_root, strategy):
	...
	meta_df_path = os.path.join(pheno_storage_root, "meta", "meta_pheno_table.csv")
	os.makedirs(os.path.dirname(meta_df_path), exist_ok=True)
	meta_df.to_csv(meta_df_path)

	meta_col_df = pd.concat([cdf[1] for cdf in pheno_dfs], axis=0)
	meta_col_df_path = os.path.join(pheno_storage_root, "meta", "meta_pheno_table_cols.csv")
