...
for combo_num in combo_nums:
    ...
    statistics_file = f"/data5/deepro/ukbiobank/papers/bmi_project/3_run_rarecomb/lifestyle_white_british/data/parsed_tables/combo_{combo_num}.csv"
    ...
    if combo_num == 4:
        app = app.melt(id_vars=['Phenotype', 'Group', 'Combo_num'], value_vars=['Item_1', 'Item_2', 'Item_3', 'Item_4'])
    ...
    app['Gene_symbol'] = app.Gene.apply(lambda s: gencode.loc[s, 'gene_name'] if s.startswith('ENSG') else np.nan)
...
df.to_csv("/data5/deepro/ukbiobank/papers/bmi_project/4_characterization/lifestyle_white_british/data/previous_associations/associations.csv", index=False)
