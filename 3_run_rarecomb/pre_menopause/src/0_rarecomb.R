...
result = compare_enrichment(df, gene_coordinates_df=gencode,combo_length=combo_length, min_indv_threshold=min_indv_threshold, max_freq_threshold=max_freq_threshold, input_format='Input_', output_format='Output_', pval_filter_threshold=pval_filter_threshold, adj_pval_type=adj_pval_type, min_power_threshold=min_power_threshold, sample_names_ind=sample_names_ind, ld_block_size=0, quiet=quiet)
